...
public class HDFC implements RBI{
    ...
    float hdfc_fd_ROI = 5, hdfc_credit_ROI = 6;
    String customer_id;
    public HDFC(BufferedReader buff, Connection connection, int check_customer) {
        ...
        if(check_customer == 1){
            ...
            int countCustomers;
            try {
                PreparedStatement book_find = connection.prepareStatement("select count(*) as customerID from hdfc where customerID = '" + customer_id + "'");
        ...
        while(check_flag) {
            ...
            switch (check_option){
                ...
                case 4 -> {
                    ...
                    HashMap<String, Float> HDFC_loan_types = new HashMap<>();
                    HDFC_loan_types.put("Home Loan", 5f);
                    HDFC_loan_types.put("Education Loan", 3f);
                    HDFC_loan_types.put("Personal Loan", 6f);
                    HDFC_loan_types.put("Car Loan", 8f);
                    ...
                    applyLoan(loan_type, loan_amount, HDFC_loan_types.get(loan_type) + rbi_loan_ROI, loan_years, customer_id);
                }
                case 5 -> {
                    ...
                    applyCreditCard(credit_amount, hdfc_credit_ROI + rbi_credit_ROI, customer_id);
                }
    ...
    public void depositMoney(float deposit_money, String customerID) {
        try {
            PreparedStatement book_find = connection.prepareStatement("select * from hdfc where customerID = '" + customerID + "'");
            ResultSet resultSet = book_find.executeQuery();
            resultSet.next();
            float balance = resultSet.getFloat(8);
            balance += deposit_money;
            PreparedStatement book_update = connection.prepareStatement("update hdfc set customerBalance = " + balance + "where customerID = '" + customerID + "'");
    ...
    public void openFD(float amount, int years) {
        ...
        for(int year = 1; year <= years; year++){
            total_fd_money += total_fd_money * (hdfc_fd_ROI / 100);
            if(flag){
    ...
    public void applyLoan(String loanType, float amount, float loan_ROI, int years, String customerID) {
        float balance;
        try {
            PreparedStatement check_loan = connection.prepareStatement("select * from hdfc where customerID = '" + customerID + "'");
    ...
    public void applyCreditCard(float amount, float credit_ROI, String customerID) {
        float balance;
        try {
            PreparedStatement check_credit = connection.prepareStatement("select * from hdfc where customerID = '" + customerID + "'");
    ...
    public float getBalance(String customerID) {
        float balance;
        try {
            PreparedStatement find_balance = connection.prepareStatement("select * from hdfc where customerID = '" + customerID + "'");
